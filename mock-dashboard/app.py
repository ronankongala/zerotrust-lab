"""mock-dashboard: a SAML 2.0 service provider for the Zero Trust lab.
...
Pair this with mock-docs (port 9001) to demonstrate SSO: visiting this app after
logging into mock-docs never prompts for credentials, because Keycloak already
has a realm session.
"""
...
SP_BASE_URL = os.environ.get("SP_BASE_URL", "http://localhost:9002").rstrip("/")
# Keycloak uses the SAML client's clientId as the SP entity ID.
SP_ENTITY_ID = os.environ.get("SP_ENTITY_ID", "mock-dashboard-saml")
...
# "localhost" cookie jar. Distinct names keep this app from reading mock-docs's
# session and appearing logged in without ever talking to Keycloak -- which
# would make the SSO demo prove nothing.
